...
class MiscController extends Controller
{
    ...
    public function show_search_page(Request $request){
    	...
    	$data['programs'] = DB::table('programs')
    							->get();
    	$data['regions'] = DB::table('m_lib_region')
    ...
    public function show_table(Request $request){
    	...
    	if($level_of_comparison == '1'){
    		...
    		$region_all = $request->input('region_all');
    		$regions = array();
    		$provinces = null;
    		$show_provinces = false;

    		if(sizeof($region_all) > 1){
    			foreach($region_all as $ra){
	    			$regions[] = DB::table('m_lib_region')
	    							->where('region_code', $ra)
	    							->first();   
	    		}
    		}else if(sizeof($region_all) == 1){
    			$region_code = ($region_all[0][0] == '0') ? $region_all[0][1] : $region_all[0];
    			
    			$provinces = DB::table('provinces')
    						->select(DB::raw('provinces.*'))
    						->distinct()
    						->join('m_lib_health_facility', 'm_lib_health_facility.psgc_provcode','=','provinces.code')
    						->where("provinces.reg_code", '=', $region_code)
    						->get();
    		...
    		$data['region_filters'] = $regions;
    		$data['region_provinces_filters'] = $provinces;
    		$data['show_provinces'] = $show_provinces;
    		$r_keys = array();
    		$r_keys_percent = array();
    		$r_link = array();
    		$r_link_percent = array();
    		$r_pr_keys = array();
    		$r_pr_keys_percent = array();
    		$r_pr_link = array();
    		$r_pr_link_percent = array();
    		$str_regions = array();
    		$str_provinces = array();
    		$all_provinces = array();
    		$all_regions = array();
    		...
    		if($show_provinces){
    			if(isset($provinces) && !empty($provinces)){
		    		...
		    		if($get_program->tbl_name == 'prog_m2_bhs'){
		    			...
		    			foreach($provinces as $province){
		    				if(strlen($province->code) <= 5){
		    					$provcode = '0' . $province->code;	
		    				}else{
		    				...
		    				if(sizeof($provinces) == 1){
		    					...
		    					if((sizeof($provinces) - 1) == $province_counter){
			    					$sql .= 'provcode = ' . $provcode;
			    				}else{
			    					$sql .= 'provcode = ' . $provcode . ' OR ';
			    				}	
		    			...
		    			foreach($get_program_result as $key => $res){
							...
							foreach($result[$provcode] as $key => $obj){
								$new_result_reg[$provcode][$obj->indicator_code] = $obj->subtotal;
								$arguments  = $this->get_arguments($provcode, $obj->indicator_code, $month1, $month2, $year, $get_program->tbl_name);
								$arguments2 = $this->get_arguments_percent($provcode, $obj->indicator_code, $month1, $month2, $year, $get_program->tbl_name);
								$link1 = url('show_graph/' .$obj->indicator_code. '/' .$month1 . '/' . $month2 . '/' . $year . '/' . $str_provinces[$provcode] . '/' . $arguments); 
								$link2 = url('show_graph/' .$obj->indicator_code. '/' .$month1 . '/' . $month2 . '/' . $year . '/' . $str_provinces[$provcode] . '/' . $arguments2);
								$r_link[$provcode][$obj->indicator_code] = $link1;
								$r_link_percent[$provcode][$obj->indicator_code] = $link2;
							}
		    			}
		    			...
		    			$data['grand_total_link'] = $grand_total;
						$data['percent'] = $new_result_reg;
						$data['whole_link'] = $r_link;
						$data['percent_link'] = $r_link_percent;
						$data['is_morbidity'] = null;
		    		}
    			}else{
    				$new_program_result = null;
    			}
    		}else{
    			foreach($regions as $region){
	    			$str_regions[$region->region_code] = $region->region_name;
	    			$all_regions[] = $region->region_code;
	    		}
	    		...
	    		if($get_program->tbl_name == 'prog_m2_bhs'){
	    			...
	    			foreach($regions as $region){
	    				$regcode = $region->region_code;
	    				/*if(strlen($region->region_code) <= 3){
	    					$regcode = '0' . $region->region_code;	
	    				}else{
	    					$regcode = $region->region_code;
	    				}*/
	    				...
	    				if(sizeof($regions) == 1){
	    					...
	    					if((sizeof($regions) - 1) == $region_counter){
		    					$sql .= 'regcode = ' . $regcode;
		    				}else{
		    					$sql .= 'regcode = ' . $regcode . ' OR ';
		    				}	
	    				}
	    			...
	    			//$str_provinces = implode("*", $str_provinces);
	    			$all_regions = implode("*", $all_regions);
	    			$sql .= 'WHERE (';
	    			foreach($regions as $region){
	    				$regcode = $region->region_code;
	    				/*if(strlen($region->region_code) == 1){
	    					$regcode = '0' . $region->region_code;	
	    				}else{
	    					$regcode = $region->region_code;
	    				}*/
	    			...
	    			foreach($get_program_result as $key => $res){
	    				$regcode = $res->regcode;
	    				/*if($res->regcode[0] == '0'){
	    					$regcode = $res->regcode[1];
	    				}else{
	    					$regcode = $res->regcode;
	    				}*/

	    				foreach($res as $key => $value){
	    					$new_program_result[$regcode][$key] = $value;
	    				}
	    				
						$result_sql  = "SELECT indicator_code, (SUM(barangay_population) * (target_pop / 100)) as subtotal FROM m_lib_psgc_code ";
						$result_sql .= "JOIN m_lib_barangay ON m_lib_barangay.barangay_id = m_lib_psgc_code.barangay_id ";
						$result_sql .= "JOIN lib_indicator ON lib_indicator.prog_id = '" . $prog_id . "' ";
						$result_sql .= "WHERE region_code = '".(($regcode[0] == '0') ? $regcode[1] : $regcode)."' GROUP BY indicator_code";
						
						$result[$regcode] = DB::select($result_sql);
						
						if(sizeof($result[$regcode]) == 0){
							$result_sql  = "SELECT indicator_code, (SUM(barangay_population) * (target_pop / 100)) as subtotal FROM m_lib_psgc_code ";
							$result_sql .= "JOIN m_lib_barangay ON m_lib_barangay.area_code = m_lib_psgc_code.barangay_id ";
							$result_sql .= "JOIN lib_indicator ON lib_indicator.prog_id = '" . $prog_id . "' ";
							$result_sql .= "WHERE region_code = '".(($regcode[0] == '0') ? $regcode[1] : $regcode)."' GROUP BY indicator_code";
						...
						foreach($result[$regcode] as $key => $obj){
							$new_result_region[$regcode][$obj->indicator_code] = $obj->subtotal;
							$arguments  = $this->get_arguments_region($regcode, $obj->indicator_code, $month1, $month2, $year, $get_program->tbl_name);
							$arguments2 = $this->get_arguments_percent_region($regcode, $obj->indicator_code, $month1, $month2, $year, $get_program->tbl_name);
							$link1 = url('show_graph/' .$obj->indicator_code. '/' .$month1 . '/' . $month2 . '/' . $year . '/' . ((strlen($regcode) == 1) ? $str_regions['0'.$regcode]: $str_regions[$regcode]). '/' . $arguments); 
							$link2 = url('show_graph/' .$obj->indicator_code. '/' .$month1 . '/' . $month2 . '/' . $year . '/' . ((strlen($regcode) == 1) ? $str_regions['0'.$regcode]: $str_regions[$regcode]). '/' . $arguments2);
							$r_link[$regcode][$obj->indicator_code] = $link1;
							$r_link_percent[$regcode][$obj->indicator_code] = $link2;

						}
	    			}
	    			
	    			foreach($indicators as $i){
	    				$link3 = url('show_graph_all/' . $i->indicator_code . '/' . $month1 . '/' . $month2 . '/' . $year . '/' . $all_regions . '/' . $get_program->tbl_name .'/region');
						$grand_total[$i->indicator_code] = $link3;
	    			}

					$data['percent'] = $new_result_region;
					$data['whole_link'] = $r_link;
					$data['percent_link'] = $r_link_percent;
					$data['grand_total_link'] = $grand_total;
					$data['is_morbidity'] = null;
    ...
    private function get_arguments_region($region_code, $indicator, $month1, $month2, $year, $table){
		$arguments = array();
		$region_code = (strlen($region_code) == 1) ? '0'.$region_code : $region_code;
		...
		$sql .= "FROM " . $table . " WHERE regcode = '" . $region_code . "' ";
		$sql .= "AND year = '" . $year . "' ";
		$sql .= 'AND month BETWEEN "' . str_pad($month1, 2, '0', STR_PAD_LEFT) . '" AND "' . str_pad($month2, 2, '0', STR_PAD_LEFT) . '"';
		$sql .= "GROUP BY month";
    ...
    private function get_arguments_percent_region($region_code, $indicator, $month1, $month2, $year, $table){
    	$arguments = array();
    	$region_code = (strlen($region_code) == 1) ? '0'.$region_code : $region_code;
    	$start_month = str_pad($month1, 2, '0', STR_PAD_LEFT);
		$end_month = str_pad($month2, 2, '0', STR_PAD_LEFT);
		...
		$get_program_result = DB::table($table)
    							...
    							->groupBy('month')
    							->get();
    	$region_code = ($region_code[0] == '0') ? $region_code[1] : $region_code;
    	foreach($get_program_result as $key => $value){
    ...
    private function get_arguments_percent($province_code, $indicator, $month1, $month2, $year, $table){
    	...
    	$province_code = ($province_code[0] == '0') ? substr($province_code, 1) : $province_code;
    	foreach($get_program_result as $key => $value){
    		$initial = ($value->$indicator == null) ? '0' : $value->$indicator;
    ...
    private function get_arguments_percent_city($citycode, $indicator, $month1, $month2, $year, $table){
		...
		$get_program_result = DB::table($table)
    							...
    							->groupBy('month')
    							->get();
    	$citycode = ($citycode[0] == '0') ? substr($citycode, 1) : $citycode;
    	foreach($get_program_result as $key => $value){
    		$initial = ($value->$indicator == null) ? '0' : $value->$indicator;
   	...
   	public function show_graph_all($indicator, $month1, $month2, $year, $places, $table, $type){
		...
		if($type == 'province'){
	    	...
	    	foreach($places as $place){
	    		$place_provcode = (strlen($place) <= 3) ? '0'. $place : $place;	
	    		$place_code = $place;
	    		$arguments[$start_month][$place_provcode] = 0.00;
				if(sizeof($places) == 1){
					$where .= 'provcode = ' . $place_provcode;
					$where2 .= 'code = ' . $place_code;
				}else{
					if((sizeof($places) - 1) == $counter){
						$where .= 'provcode = ' . $place_provcode;
						$where2 .= 'code = ' . $place_code;
					}else{
						$where .= 'provcode = ' . $place_provcode . ' OR ';
						$where2 .= 'code = ' . $place_code . ' OR ';
					}	
				}
			...
			$where2 = "WHERE (";
	    	foreach($places as $place){
	    		$place_citycode = (strlen($place) <= 5) ? '0'. $place : $place;	
	    		$place_code = $place;
	    		$arguments[$start_month][$place_citycode] = 0.00;
				if(sizeof($places) == 1){
					$where .= 'citycode = ' . $place_citycode;
					$where2 .= 'code = ' . $place_code;
				}else{
					if((sizeof($places) - 1) == $counter){
						$where .= 'citycode = ' . $place_citycode;
						...
						$where .= 'citycode = ' . $place_citycode . ' OR ';
						$where2 .= 'code = ' . $place_code . ' OR ';
					}	
				}
			...
			$where2 = "WHERE (";
	    	foreach($places as $place){
	    		$place = (strlen($place) == 1) ? '0'. $place : $place;	
	    		$arguments[$start_month][$place] = 0.00;
				if(sizeof($places) == 1){
					$where .= 'regcode = ' . $place;
					$where2 .= 'region_code = ' . $place;
				}else{
					if((sizeof($places) - 1) == $counter){
						$where .= 'regcode = ' . $place;
						$where2 .= 'region_code = ' . $place;
					}else{
						$where .= 'regcode = ' . $place . ' OR ';
						$where2 .= 'region_code = ' . $place . ' OR ';
					}	
				}
		    ...
		    $regions = DB::select('SELECT * FROM m_lib_region ' . $where2);
		    $data['graph_data'] = array();
		    $data['graph_names'] = array();
		    foreach($regions as $region){
		    	$region->region_code = (strlen($region->region_code) == 1) ? '0'. $region->region_code : $region->region_code;	
		    	$data['graph_data'][$region->region_code] = $region->region_name;
		    	$data['graph_names'][] = $region->region_name;
		    }

		    foreach($months as $m){
		    	$sql  = "SELECT regcode, SUM(" . $indicator . ") as " . $indicator . " ";
				$sql .= "FROM " . $table . " ";
				$sql .= $where;
				$sql .= "AND year = '" . $year . "' ";
				$sql .= 'AND month = "' . $m . '" ';
				...
				foreach($result as $key => $value){
					$arguments[$m][$value->regcode] = floatval($value->$indicator);
				}
		    }
		}
    ...
    public function process_map(Request $request){
		...
		if($exploded_case[0] == 'morbidity'){
			...
			$lib_indicator_where = array('indicator_code' => $first_indicator);
			$lib = DB::table('lib_indicator')
						->where($lib_indicator_where)
						->first();
			...
			foreach($facilities as $f){
	    		...
	    		$get_target_population = DB::table('m_lib_barangay')
											 ...
											 ->first();*/
											 ->select(DB::raw('barangay_population as population'))
											 ->join('m_lib_health_facility', 'm_lib_health_facility.psgc_brgycode', '=' , 'm_lib_barangay.barangay_id')
											 ->where('facility_id', $f->facility_id)
											 ->first();
				
    			if(isset($get_target_population->population) && !empty($get_target_population->population)){
					$population_target = $get_target_population->population;
    				if($population_target == 0){
	    				...
	    				if($get_result->total == null){
	    					...
	    					$total = ((($get_result->total) / ($population_target * ($lib->target_pop/100))) * 100);
	    					//$total = ($get_result->total / $get_target_population->population) * 100;
	    				}
	    			}
    			}else{
    				$total = 0;
			...
			$lib_indicator_where = array('indicator_code' => $exploded_case[1]);
			$lib = DB::table('lib_indicator')
						->where($lib_indicator_where)
						->first();
			$facilities = DB::table('m_lib_health_facility')
			...
			if($exploded_case[1] == 'completeness'){
				...
				$data['legend'] = $str;
			}else if($exploded_case[1] == 'timeliness'){
				foreach($facilities as $f){
					$month = ($month[0] == '0') ? substr($month, 1) : $month;
					$where2 = array('year' => $year,
									'month' => $month,
				...
				foreach($facilities as $f){
	    			$result[$f->doh_class_id] = $this->get_level_color($f->level);
	    			$result_details[$f->doh_class_id]['name'] = $f->facility_name;
	    			$result_details[$f->doh_class_id]['longitude'] = $f->longitude;
	    			$result_details[$f->doh_class_id]['latitude'] = $f->latitude;
	    			$result_details[$f->doh_class_id]['label'] = '<span>' . $f->facility_name . '</span><br>';
	    		...
	    		$data['details'] = $result_details;
	    		$data['error'] = (sizeof($data['result']) > 0) ? null : 'No results found.';
				$data['wah_type'] = 'wah_level';
				$str = '';
				$str  = '<div style="text-align:center;font-size:0.8em;"><strong>Facility Levels Legend</strong></div>';
    			$str .= '<div><img src="' . asset('public_html/imgs/blue_box.png') . '" style="width:30px;height:30px;"/> Level 1</div>';
    			$str .= '<div><img src="' . asset('public_html/imgs/yellow_box.png') . '" style="width:30px;height:30px;"/> Level 2</div>';
    			$str .= '<div><img src="' . asset('public_html/imgs/green_box.png') . '" style="width:30px;height:30px;"/> Level 3</div>';
    			$str .= '<div><img src="' . asset('public_html/imgs/violet_box.png') . '" style="width:30px;height:30px;"/> Level 4</div>';
				$data['legend'] = $str;
    ...
    public function get_completeness_condition_label($count, $result, $level){
		...
		if(($level == 2) || ($level == 3) || ($level == 4)){
			if($count == 4){
				$label = '<span><b>Level '.$level.'</b></span><br>';
			...
			if($count > 0){
				...
				if(in_array(2, $get_program_fks)){
					...
					$label .= '<span><b>Complete(1)</b></span><br>';
    ...
    public function post_home(Request $request){
    	if($request->session()->has('id')){
    		...
    		$where = array('month' => $month,
						   'year' => $year);
    		$fic_sql  = 'SELECT facility_name, SUM(barangay_population), (FIC_M + FIC_F) as fic_total, target_pop, (((FIC_M + FIC_F) / ROUND((barangay_population * (target_pop/100)), 2)) * 100) as total FROM prog_child_care ';
    		$fic_sql .= 'JOIN m_lib_health_facility ON m_lib_health_facility.doh_class_id = prog_child_care.HFHUDCODE ';
    		$fic_sql .= 'JOIN m_lib_barangay ON m_lib_barangay.barangay_id = prog_child_care.BGYCODE ';
    		$fic_sql .= 'JOIN lib_indicator ON lib_indicator.indicator_code = "FIC_M" ';
