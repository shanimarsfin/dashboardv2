...
->orderBy("year", 'desc')
		...
		if($exploded_case[0] == 'morbidity'){
			...
			if($exploded_case[1] == 'completeness'){
				foreach($facilities as $f){
	    			...
	    			$result[$f->doh_class_id] = $this->get_completeness_condition($get_result->count());
	    			...
	    			$result_details[$f->doh_class_id]['label'] .= '<span>' . $this->get_completeness_condition_label($get_result->count(), $get_result->get()) . '</span><br>';
				}
    ...
    public function get_completeness_condition($count){
    	$color = '';
    ...
    public function get_completeness_condition_label($count, $result){
    	$label = '';
    	$counter = 0;
